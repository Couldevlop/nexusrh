...
function bars(slices: Slice[]): string {
  if (slices.length === 0) return '<p style="color:#64748b">Aucune donnee sur la periode.</p>'
...
export function renderHtml(data: ReportData, a: Analysis): string {
  ...
  const entreprises = data.tenants.length === 0
    ...
    : `<table style="${CSS_TABLE}">
        <tr><th style="${CSS_TH}">Entreprise</th><th style="${CSS_TH}">Effectif</th>
            <th style="${CSS_TH}">Arrivees</th><th style="${CSS_TH}">Departs</th>
            <th style="${CSS_TH}">Connectes</th><th style="${CSS_TH}">Echecs</th></tr>
        ${data.tenants.map((x) => `
  ...
  const cabinets = data.agencies.length === 0
    ? '<p style="color:#64748b">Aucun cabinet enregistre.</p>'
    : `<table style="${CSS_TABLE}">
        <tr><th style="${CSS_TH}">Cabinet</th><th style="${CSS_TH}">Entreprises</th>
            <th style="${CSS_TH}">Effectif cumule</th><th style="${CSS_TH}">Rattachees</th>
            <th style="${CSS_TH}">Detachees</th></tr>
        ${data.agencies.map((c) => `
  ...
  const alertes = a.alerts.length === 0
    ? '<p style="color:#15803d">Aucun point d\'attention sur la periode.</p>'
  ...
  <p style="color:#64748b">NexusRH CI · OpenLab Consulting · le detail complet et les graphiques sont dans le PDF joint.</p>
  ...
  <table style="${CSS_TABLE}">
    ...
    <tr><td style="${CSS_TD}">Nouvelles sur la periode</td><td style="${CSS_TD}"><strong>${t.newTenants}</strong></td></tr>
    <tr><td style="${CSS_TD}">Effectif consolide</td><td style="${CSS_TD}"><strong>${t.headcount}</strong> — ${t.hires} arrivees, ${t.departures} departs</td></tr>
    <tr><td style="${CSS_TD}">Connexions</td><td style="${CSS_TD}"><strong>${t.loginSuccess}</strong> reussies, ${t.loginFailed} echouees, ${t.loginLocked} verrouillages</td></tr>
  </table>
  ...
  <h2 style="font-size:16px">Connexions reussies par jour</h2>
  ${bars(a.loginsByDay)}
