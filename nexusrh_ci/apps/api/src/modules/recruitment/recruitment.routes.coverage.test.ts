...
describe('POST /recruitment/jobs/:id/source/compare — branches restantes', () => {
  ...
  it('utilise des plateformes par défaut quand body.platforms vide (ligne 1664)', async () => {
    ...
    vi.mocked(sourceProfilesCompare).mockResolvedValueOnce({
      winner: 'mistral',
      claude: { provider: 'claude', model: 'c', label: 'Claude', data: null, jsonValid: true, richnessScore: 50, profilesGenerated: 2, latencyMs: 100, inputTokens: 1, outputTokens: 1, estimatedCostEur: 0.01, truncated: false, error: null },
      mistral: { provider: 'mistral', model: 'm', label: 'Mistral', data: null, jsonValid: true, richnessScore: 70, profilesGenerated: 2, latencyMs: 90, inputTokens: 1, outputTokens: 1, estimatedCostEur: 0.005, truncated: false, error: null },
...
describe('POST /recruitment/jobs/:id/source/compare — platforms explicites', () => {
  it('transmet les platforms du body quand fournis (ligne 1664)', async () => {
    ...
    vi.mocked(sourceProfilesCompare).mockResolvedValueOnce({
      winner: 'claude',
      claude: { provider: 'claude', model: 'c', label: 'Claude', data: null, jsonValid: true, richnessScore: 70, profilesGenerated: 2, latencyMs: 100, inputTokens: 1, outputTokens: 1, estimatedCostEur: 0.01, truncated: false, error: null },
      mistral: { provider: 'mistral', model: 'm', label: 'Mistral', data: null, jsonValid: true, richnessScore: 50, profilesGenerated: 2, latencyMs: 90, inputTokens: 1, outputTokens: 1, estimatedCostEur: 0.005, truncated: false, error: null },
      ratios: { latency: 'l', cost: 'c', richness: 'r' },
