...
export function createTenantSchema(schemaName: string) {
  ...
  const recruitmentJobs = schema.table('recruitment_jobs', {
    ...
    // Visibilité & ciblage interne (offres internes / externes / les deux)
    visibility:       varchar('visibility', { length: 20 }).default('external'),
    targetDepartments: text('target_departments').array(),
    targetJobLevels:  text('target_job_levels').array(),
    targetMinSeniorityMonths: integer('target_min_seniority_months'),
  ...
  const applications = schema.table('applications', {
    ...
    aiStrengths:        jsonb('ai_strengths').default('[]'),
    aiGaps:             jsonb('ai_gaps').default('[]'),
    aiRedFlags:         jsonb('ai_red_flags').default('[]'),
    aiInterviewQuestions: jsonb('ai_interview_questions').default('[]'),
    aiModelUsed:        varchar('ai_model_used', { length: 30 }),
