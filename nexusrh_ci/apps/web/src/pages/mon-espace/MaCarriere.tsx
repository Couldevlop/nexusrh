...
export default function MaCarriere() {
  ...
  const typeLabel = (type: string) => t(`career.evaluationType.${type}`, { defaultValue: type })
  const statusLabel = (status: string) => t(`career.evaluationStatus.${status}`, { defaultValue: status })
  ...
  const skillsByCategory = skills.reduce<Record<string, Skill[]>>((acc, s) => {
    const cat = s.category ?? t('career.defaultCategory')
    acc[cat] = acc[cat] ?? []
    acc[cat].push(s)
    return acc
  }, {})
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">{t('career.subtitle')}</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        {loadingEvals ? (
          ...
          <p className="text-sm text-muted-foreground">{t('career.noUpcoming')}</p>
        ) : (
            ...
            {upcoming.map(ev => (
              <div key={ev.id} className="flex items-center justify-between rounded-lg border border-border p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{typeLabel(ev.type)}</p>
                    <p className="text-xs text-muted-foreground">
                      {ev.evaluator_first_name
                        ? t('career.evaluator', { name: `${ev.evaluator_first_name} ${ev.evaluator_last_name}` })
                        : t('career.evaluatorUnassigned')} · {ev.year}
                    </p>
                  </div>
                </div>
            ...
            {history.map(ev => (
              <div key={ev.id} className="rounded-lg border border-border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-sm">{typeLabel(ev.type)} — {ev.year}</p>
                    {ev.completed_at && (
                      <p className="text-xs text-muted-foreground">{t('career.finalizedOn', { date: formatDate(ev.completed_at) })}</p>
                    )}
                  </div>
                  {ev.global_score && (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <ScoreBar label={t('career.scorePerformance')} value={ev.performance_score ? parseFloat(ev.performance_score) : null} />
                  <ScoreBar label={t('career.scoreSkills')} value={ev.skills_score ? parseFloat(ev.skills_score) : null} />
                </div>
                {ev.notes && (
                    ...
                    <div key={sk.skill_name} className="space-y-1.5">
                      <div className="flex justify-between text-sm">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t('career.skillLevel', { level: sk.level })}</span>
                          {sk.target_level && sk.target_level > sk.level && (
                            <span className="flex items-center gap-0.5 text-primary">
                              <TrendingUp className="h-3 w-3" />
                              {t('career.skillTarget', { target: sk.target_level })}
                            </span>
                          )}
                        </div>
                      </div>
      ...
      {skills.length === 0 && history.length === 0 && !loadingEvals && (
        <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
          ...
          <p className="font-medium">{t('career.emptyTitle')}</p>
          <p className="text-sm mt-1">{t('career.emptyHint')}</p>
        </div>
      )}
    </div>
  )
