...
const schema = z.object({
  absenceTypeId: z.string().min(1, 'Type requis'),
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Gestion de mes congés CI (Code du Travail)</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {balances.map(b => (
          <div key={b.absence_type_id} className="rounded-xl border border-border bg-card p-4">
            ...
            <p className="text-xl font-bold">{b.remaining}j</p>
            <p className="text-xs text-muted-foreground">restants / {b.acquired}j acquis</p>
          </div>
        ))}
      </div>
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-6">
          <h2 className="font-semibold mb-4">Nouvelle demande d'absence</h2>
          <form onSubmit={handleSubmit(data => createMut.mutate(data))} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                {errors.absenceTypeId && <p className="text-xs text-destructive mt-1">{errors.absenceTypeId.message}</p>}
              </div>
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="halfDay" className="text-sm">Demi-journée</label>
              </div>
              <div className="col-span-2">
                <label className="text-sm font-medium">Motif (optionnel)</label>
                <textarea {...register('reason')} rows={2} placeholder="Précisez le motif..."
            ...
            {createMut.isError && (
              <p className="text-sm text-destructive">
                {(createMut.error as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Erreur'}
              </p>
            )}
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="border-b border-border px-6 py-4">
          <h2 className="font-semibold">Historique de mes absences</h2>
        </div>
        {isLoading ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {' · '}{abs.half_day ? '½ journée' : `${abs.days} jour(s)`}
                    </p>
