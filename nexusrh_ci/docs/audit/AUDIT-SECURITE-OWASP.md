...
| Faible | `apps/api/src/modules/auth/auth.routes.ts:316,538` | MFA obligatoire (super_admin/tenant) **désactivé par défaut** (paramétrable). Choix produit assumé (débloque la création de tenant). | Faible si activé en prod sur les comptes privilégiés. | Recommander/forcer le MFA pour `super_admin` et `admin` en production. → ✅ **Résolu (PR #206/#207)** : `mfa_required_super_admin` **et** `mfa_required_tenant_users` sont à `true` en production, avec parcours d'enrôlement `/mfa-setup` non contournable. |
